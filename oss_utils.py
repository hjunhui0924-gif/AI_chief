...
def handle_image_upload(img_bytes: bytes, filename: str, content_type: str) -> str:
    ...
    if oss_access_key and oss_secret and oss_bucket:
        try:
            ...
            # 获取可访问的网络URL
            oss_endpoint_clean = oss_endpoint.replace("https://", "").replace("http://", "")
            img_url = f"https://{oss_bucket}.{oss_endpoint_clean}/{object_name}"
            logger.info(f"图片成功上传至OSS: {img_url}")
            return img_url
    ...
    # 如果没配置OSS环境变量或上传失败，回退为Base64供模型读取
